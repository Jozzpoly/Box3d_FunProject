...
- **F-46** · `VEHICLE_MEASURED` — B3WHEEL-STEER-01: w pełnym headless M6 po jednoczesnym puszczeniu gazu i kierownicy b3Wheel wraca w obu kierunkach dla wejścia ±0.40, lecz dla ±0.50 przechodzi w stan ANTI_CENTERING_OR_HOLD. Przejście jest tylko ograniczone zbadanym przedziałem 0.40–0.50; dokładny punkt nie został wyznaczony. Przy ±1.00 koła nie schodzą poniżej 3° w ciągu 4 s i pojazd nadal porusza się około 4 m/s.
- **F-47** · `VEHICLE_MEASURED` — B3WHEEL-STEER-01 reference envelopes nie tworzą prostego porządku „stare dobre, nowe złe”: w tym samym protokole torus wraca obustronnie, natomiast sfera jest kierunkowo asymetryczna — +1.00 wraca, a -1.00 pozostaje przy około -34° podczas dalszego ruchu.
- **F-48** · `VEHICLE_MEASURED` — B3WHEEL-STEER-01B: gęsty sweep pełnego headless M6 wyznacza dwa osobne monotoniczne przedziały przejścia tego rigu — skręt dodatni wraca przy +0,44 i nie wraca przy +0,45, a ujemny wraca przy -0,41 i nie wraca przy -0,42.
- **F-49** · `VEHICLE_MEASURED` — B3WHEEL-STEER-01B: ogranicznik twist przegubu kulowego nie inicjuje granicznego antycentrowania. Dla +0,45 trwałe przejście zaczyna się przy 0,5000 s, a limit wchodzi przy 0,7333 s; dla -0,42 odpowiednio 0,4167 s i 0,6333 s.
- **F-50** · `VEHICLE_MEASURED` — B3WHEEL-STEER-01B: w chwili trwałego przejścia reakcja spin-joint na zwrotnicę koła niosącego dominujący impuls normalny ma znak zgodny z dalszym skrętem, podczas gdy motor racka i moment dominującego drążka działają przeciwnie. Geometryczny środek impulsu normalnego leży wtedy bardziej z przodu osi skrętu niż w sąsiednim przypadku wracającym.
