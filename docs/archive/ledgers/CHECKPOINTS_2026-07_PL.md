...
- EFEKT:  Bramka zielona (build 3/3, walidator OK z 5 nowymi asercjami `fragment`, test PASS, smoke 0 err). RENDER (2 zrzuty): sekcja „Spawn pojazdu" na Płycie pokazuje „Aktywny spawn (0.0,0.0) — źródło: wbudowany"; po podłożeniu testowego pliku `plate=50,60` boot headless czyta „(50.0,60.0) — źródło: domyślny", pola seedują 50/60, „Usuń domyślny" się pojawia (plik testowy skasowany — rodzi się dopiero przy pierwszym zapisie Jozza). „R" bez zmian (checkpoint), zero regresji.
- DALEJ:  ZADANIE 2 (osobno): selektor wielu skanów w UI — enumeracja katalogu-korzenia (przyszły `JOZZ_SCAN_LIBRARY`) → lista → `LoadScanTile(chosen)`. Szew gotowy.

## 2026-07-24 · Skan M2: teksturowany mesh renderu (skóra wyspy) · jozz-scan-terrain-f0
- CO:     Nowy moduł render-only `jozz_vehicle_scan_visual.{h,cpp}` (lustro do geometry=kolizja): własny parser JSPREV2 zachowujący pos+normal+uv per GRUPA (25 grup), dekoduje PNG baseColor każdej grupy i rejestruje teksturowany mesh GPU tą samą ścieżką co nadwozie (`RegisterTexturedMesh`→`AppendMesh`, `MESH_MATERIAL_MODE_TEXTURED`). Ładowany z TEJ SAMEJ paczki i TEGO SAMEGO `placement.origin` co kolider (zero dryfu), nawinięcie źródłowe (flip=false). Kolider chowany z debug-draw gdy skóra wczytana (`SetShapeHidden` na złapanym `terrainShape`), przełącznik „Pokaż siatkę kolizji". Duplikat sekcji „Mapa" usunięty ze „Świata".
- CZEMU:  Skan był collision-only (widoczny tylko przez druciany debug-draw). Jozz: zaimplementuj tekstury, zfinalizuj; „był już renderer i tekstury działały, płynnie — zrób POWTARZALNY system, żeby kolejne skany wchodziły prosto i szybko". Stąd świadome stanie na istniejącym: format pipeline'u + renderer box3d + jedno wejście `JOZZ_SCAN_PREVIEW_PACK`.
- EFEKT:  RENDER zwalidowany (3 zrzuty): wyspa jak fotorealistyczny krajobraz (wioska, droga, drzewa, pola), tekstury 25/25 poprawnie zmapowane, zero druta po schowaniu kolidera, 0 sokol err. Ładowanie skóry: Release 629ms, Debug 1158ms (100 MB GPU) → cały load Release ~1.4s / Debug ~2.3s (cel Jozza „2-3s z teksturami" trafiony). Bramka zielona (build 3/3, walidator OK, test PASS, smoke 0 err).
- DALEJ:  Powtarzalny kanał: pipeline→nowa paczka→env→„Wczytaj skan" ładuje kolizję+skórę razem, cache kluczuje po paczce. M4 reader GLB + role Structure/Vegetation dla czystych rozdzielonych skanów. Do rozważenia: cache dekodowanych tekstur, 7 zakładek top-level się przewija.
...
- EFEKT:  Dokumentacja jawnie rozdziela „kod działa” od „mapa zaakceptowana”, dodaje footprint/approach/runoff validator, neutralny język wizualny, skeleton STOP-gate i obowiązkowe porównanie przed/odrzucony/nowy; kod mapy pozostaje bez zmian w tym docs-only rebasie.
- DALEJ:  E2R.0 zachowuje baseline, E2R.1 buduje kontrakt danych i walidator; przed bryłami obowiązkowa akceptacja skeletonu centralnego kampusu przez Jozza.

## 2026-07-12 · Etap 2 mapy: obstacle kit + poligon zawieszeń 6 lane'ów · b8afab9
...
- DALEJ:  **HISTORYCZNE / NADPISANE decyzją powyżej:** Etap 3 jest zablokowany; najpierw E2R i centralny kampus.
