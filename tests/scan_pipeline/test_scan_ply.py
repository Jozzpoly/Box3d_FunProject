...
def build_ply(
    ...
    *,
    ...
    if format_name == "ascii":
        body = "".join(f"{x} {y} {z} {r} {g} {b}\n" for x, y, z, r, g, b in points).encode("ascii")
    else:
...
class ScanPlyTests(unittest.TestCase):
    def test_little_endian_counts_bounds_rgb_and_hash(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            report = scan_ply.inspect_ply(path, "safe/MipTile_0.ply", prefer_numpy=False, chunk_vertices=1)
    ...
    def test_chunks_are_bounded_and_deterministic(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            first = list(scan_ply.iter_vertex_chunks(path, header, chunk_vertices=3, prefer_numpy=False))
            second = list(scan_ply.iter_vertex_chunks(path, header, chunk_vertices=3, prefer_numpy=False))
    ...
    def test_comments_and_obj_info_are_not_copied(self) -> None:
        ...
        payload = payload.replace(b"element vertex", f"obj_info {secret}\nelement vertex".encode("ascii"))
        with tempfile.TemporaryDirectory() as temporary:
    ...
    def test_ascii_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            path.write_bytes(build_ply([(0.0, 0.0, 0.0, 0, 0, 0)], format_name="ascii"))
